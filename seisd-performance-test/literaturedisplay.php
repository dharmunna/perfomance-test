<?php
    ...
    echo "<a href='homepage.php'>HOME</a> | <a href='catagoryview.php'>VIEW BOOK</a> | <a href='comicsdisplay.php'>COSMICS</a> | <a href='nobledisplay.php'>NOBLE</a>";
...
<head>
    ...
    <style>
        body{
            margin: 10% auto 0;font-size: 30px; background-color: #B49EB2;
        }
        td{
            text-align: center;
        }
...
>
    ...
    /* Fetch data from databse(select query) */
            $res = $con->query("select * from literaturebook") or die(mysqli_error($con));
...
<?php
    /* Delete code.Wehen click on delete link this will perform.!*/
        ...
        {$query2="select RegProfile from literaturebook where bookname='".$_REQUEST["delete"]."'";
            ...
            $query3="delete from literaturebook where bookname='".$_REQUEST["delete"]."'";
            mysqli_query($con,$query3) or die(mysqli_error($con));
            echo "<script>alert('Data deleted successfully..!');window.location='literaturedisplay.php';</script>";   
        }
    ?>
</body>
</html>